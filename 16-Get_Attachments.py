...
def get_all_item_ids(json_file_path):
    try:
        with open(json_file_path, 'r', encoding="utf-8") as json_file:
            ...
            # בדיקה אם הנתונים הם רשימה של אובייקטים וסינון IDs גדולים מ-5318
            if isinstance(data, list):
                    return [item.get("JsonID") for item in data if "JsonID" in item and item.get("JsonID") > 8200]
            else:
...
# פונקציה ראשית
def main():
    json_file_path = 'work_items_match3.json'
    ...
    if all_attachments:
        with open('attachments.json', 'w') as json_file:
            json.dump(all_attachments, json_file, indent=4)
            print("All attachment data has been saved to 'attachments.json'.")
    else:
